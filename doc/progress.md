...
### 核心修复
1. **smartctl 回退链路无黑窗**：已在 Rust 后端集成 smartctl 命令行回退，使用 `CREATE_NO_WINDOW` 标志避免控制台窗口闪现
2. **内存细分指标全链路**：通过 WMI 查询实现内存缓存、提交、分页池等详细指标的完整数据流
3. **GPU VRAM 总量与使用率%**：前后端类型已对齐，支持 `vram_total_mb` 和 `vram_usage_pct` 字段
